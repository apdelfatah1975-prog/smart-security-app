...
import { BarChart3, CircleDollarSign, LayoutGrid, ShieldCheck, WalletCards } from "lucide-react";
...
const bottomItems = [
  ...
  { path: "/debts", label: "الديون", icon: WalletCards },
...
export default function BottomNavigation() {
  ...
  return (
    <nav aria-label="التنقل السريع" className="bottom-navigation fixed inset-x-3 bottom-3 z-30 grid grid-cols-5 gap-1 rounded-[1.35rem] border border-white/70 bg-white/95 p-1.5 shadow-[0_18px_45px_rgba(15,23,42,.18)] backdrop-blur-xl lg:hidden">
      {bottomItems.map(item => {
        const Icon = item.icon;
        const active = item.path === "/more" ? !bottomItems.slice(0, 4).some(entry => entry.path === location) && location !== "/" : location === item.path;
