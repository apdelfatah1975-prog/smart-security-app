...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div dir="rtl" className="min-h-[100dvh] bg-[#f6fbfa] text-slate-950">
    <aside className={`fixed inset-y-0 right-0 z-40 w-72 transform bg-[#073b4c] p-5 text-white shadow-2xl transition-transform duration-200 lg:translate-x-0 ${open ? "translate-x-0" : "translate-x-full"}`}>
      ...
      <div className="absolute bottom-5 left-5 right-5 rounded-2xl bg-white/10 p-4"><p className="text-xs font-black text-teal-100">وضع التشغيل</p><p className="mt-1 font-black">سحابي مع نسخة احتياطية محلية</p><p className="mt-2 text-xs leading-5 text-teal-100/70">تُزامن السجلات عبر PostgreSQL عند تسجيل الدخول، وتظل النسخة المحلية متاحة عند انقطاع الاتصال.</p></div>
    </aside>
