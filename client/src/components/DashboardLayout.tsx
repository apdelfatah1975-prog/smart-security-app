...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div dir="rtl" className="app-shell min-h-[100dvh] bg-[#f4f8f7] text-slate-950">
      <aside className={`fixed inset-y-0 right-0 z-40 flex w-[18rem] flex-col bg-[linear-gradient(160deg,#063b48_0%,#07545a_58%,#0f766e_100%)] p-5 text-white shadow-[0_0_50px_rgba(7,59,76,.18)] transition-transform duration-200 lg:translate-x-0 ${open ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <nav className="mt-8 flex-1 space-y-2" aria-label="القائمة الرئيسية">
          {items.map(item => {
        ...
        <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur-sm"><p className="text-xs font-black text-teal-100">وضع التشغيل</p><p className="mt-1 font-black">سحابي مع نسخة احتياطية محلية</p><p className="mt-2 text-xs leading-5 text-teal-100/70">تُزامن السجلات عبر PostgreSQL، وتظل النسخة المحلية متاحة عند انقطاع الاتصال.</p></div>
      </aside>
